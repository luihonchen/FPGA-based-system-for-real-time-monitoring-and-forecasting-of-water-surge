...
# HPS code
libtensorflow-lite.a                -> tensorflow lite library version 2.4.1 that can supported by HPS

avalon_mm_hps.cpp                   -> real-time read sensor reading from memory (get the sensor reading, FPGA latency, adc_volage)

dataset_test_accuracy.cpp           -> test the LSTM model latency, accuracy using the test dataset

run_lstm_prediction.cpp             -> get the sensor reading from memory, run prediction, threshold comparison and database upload
